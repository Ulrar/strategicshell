...
renderBody zoom offset ss (Body x y _ _ c r b) = move (inGameToScreenCoord x y offset ss zoom) $ filled c $ circle (r * zoom)
...
view (Model lsolsys dsi ss offset zoom) = Graphics2D $ collage
  (if (L.length lsolsys) - 1 < dsi then [] else [renderBodies zoom offset ss ([sun $ lsolsys L.!! dsi])])
